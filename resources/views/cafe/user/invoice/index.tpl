...
<div class="mb-6">
    <h2 class="text-2xl font-semibold tracking-tight">账单记录</h2>
    <p class="text-faint mt-1 text-sm">查看账户中的账单与支付状态</p>
</div>

<div x-data="cafeTable('/user/invoice/ajax', 'invoices')" class="c-card">
    <div class="flex flex-wrap items-center justify-between gap-3 p-5 pb-3">
        <h3 class="text-base">全部账单</h3>
        <input type="search" x-model="search" @input="page = 1" placeholder="搜索账单…"
               class="field-input !w-56">
    </div>

    <div class="table-card overflow-x-auto">
        <table data-column-storage="cafe.user.invoice.index.columns.v1" data-column-widths="114,114,130,160,220,220,110" data-column-minimums="114,114,80,110,160,160,110" data-fixed-last>
            <thead>
            <tr>
                <th>账单 ID</th>
                <th>订单 ID</th>
            ...
            <template x-for="row in paged" :key="row.id">
                <tr>
                    ...
                    <td class="text-faint" x-text="row.pay_time"></td>
                    <td>
                        <div class="flex justify-end">
                            <a class="btn-secondary btn-sm" :href="'/user/invoice/' + row.id + '/view'">查看</a>
                        </div>
                    </td>
                </tr>
            </template>
