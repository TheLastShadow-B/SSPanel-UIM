...
<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h2 class="text-2xl font-semibold tracking-tight">公告</h2>
        <p class="text-faint mt-1 text-sm">站点公告管理</p>
    </div>
    <a href="/admin/announcement/create" class="btn-primary btn-sm"><i class="ti ti-plus"></i> 发布公告</a>
</div>

<div x-data="cafeTable('/admin/announcement/ajax', 'anns')" class="c-card">
        ...
        <table>
            ...
            <template x-for="row in paged" :key="row.id">
                <tr>
                    ...
                    <td class="max-w-64 truncate" x-text="row.content"></td>
                    <td><div class="flex justify-end gap-1.5">
                        <a class="btn-secondary btn-sm" :href="'/admin/announcement/' + row.id + '/edit'">编辑</a>
                        <button class="btn-danger-soft btn-sm" @click="destroy('/admin/announcement/' + row.id, '确定删除此公告？', row.id)">删除</button>
                    </div></td>
                </tr>
            </template>
            </tbody>
