{include file='shell/admin_header.tpl' nav='log-money'}

<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h2 class="text-2xl font-semibold tracking-tight">余额日志</h2>
        <p class="text-faint mt-1 text-sm">用户余额变动记录</p>
    </div>
    <span></span>
</div>

<div x-data="cafeTable('/admin/money/ajax', 'money_logs')" class="c-card">
    ...
    <div class="table-card overflow-x-auto">
        <table data-column-storage="cafe.admin.log.money.columns.v1" data-column-widths="100,100,130,130,130,320,220" data-column-minimums="100,80,86,86,100,160,160">
            <thead>
            <tr>
                <th>事件ID</th>
                <th>用户</th>
            ...
            <template x-for="row in paged" :key="row.id">
                <tr>
                    ...
                    <td x-text="row.before"></td>
                    <td x-text="row.after"></td>
                    <td class="text-ink font-medium" x-text="'¥ ' + row.amount"></td>
                    <td class="max-w-64 truncate" x-text="row.remark"></td>
        ...
        <div x-show="!loading && paged.length === 0" x-cloak
            ...
            <i class="ti ti-coin-off text-2xl"></i>
            暂无数据
        </div>
    </div>
